...
class TestSmsClientGetTemplateAbnorQa(unittest.TestCase):
    data = [
            #templateId为None
    ...
    dataDeletedTemplate = [
                        [0, 'x', 'y', 'ServerError', 'status=403, message=no records in database'],
                        ]
    ...
    def test(self):
        for tag, tId, exceptType, exceptMsg in self.data:
            print tag   #debug
            self.assertRaisesRegexp(eval(exceptType), exceptMsg, self.sms.get_template, tId)
