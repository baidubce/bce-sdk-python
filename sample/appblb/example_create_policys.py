...
"""
Example for create appblb policys.
"""
...
if __name__ == '__main__':
    ...
    try:
        ...
        app_policy = {
            'desc': 'for test',        # policy的描述
            'appServerGroupId': 'Your server group id',  # 服务器组的id
            'backendPort': 80,         # 后端服务端口
            'priority': 100,           # 后端服务权重
            'ruleList': rule_list      # 规则列表
        }
        app_policy_vos.append(app_policy)
        blb_id = 'Your Blbid'  # 指定的BLB ID
        listener_port = 80     # 监听器的监听端口
        resp = app_blb_client.create_policys(blb_id=blb_id, listener_port=listener_port,
