...
if __name__ == "__main__":
    ...
    memberList = [
        {
            "memberId": "Your Ipmemberid",  # IP组成员ID
            "weight": 70,  # 权重
            "port": 80  # 端口
        }
    ]
    ...
    try:
        # update ip group member
        resp = app_blb_client.update_app_ip_group_member(blb_id, ip_group_id, memberList)
        print("[example] update ip group member response :%s" % resp)
